	...
	protected void onDraw(Canvas canvas) {
		...
		for (int iLoop = 0; iLoop < count; iLoop++) {
			...
			if (pageFillRadius != mRadius) {
				canvas.drawRect(new RectF(dX - mRadius / 2, dY - mRadius / 2,
						dX + mRadius / 2, dY + mRadius / 2), mPaintStroke);
			}
		}
		...
		canvas.drawRect(new RectF(dX - mRadius / 2, dY - mRadius / 2, dX
				+ mRadius / 2, dY + mRadius / 2), mPaintFill);
	}
